...
| `install-azurecli.sh` | Installs the [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/) via the Microsoft apt repository | No (uses sudo) |
...
| `install-dev-dependencies.sh` | Installs C build toolchain and headers for Rust FFI crates (build-essential, pkg-config, libssl-dev, libvirt-dev, etc.) | No (uses sudo) |
| `install-docker.sh` | Installs Docker Engine, CLI, Buildx, and Compose via the official Docker apt repository | No (uses sudo) |
| `install-githubcli.sh` | Installs the GitHub CLI (`gh`) via the official apt repository | No (uses sudo) |
...
| `install-packer.sh` | Installs HashiCorp Packer via the official apt repository | No (uses sudo) |
| `install-pi.sh` | Installs [pi](https://pi.dev) (Pi Coding Agent) globally via npm | No (requires Node.js) |
| `install-qemu-guest-agent.sh` | Installs and enables the [QEMU Guest Agent](https://wiki.qemu.org/Features/GuestAgent) service | No (uses sudo) |
...
| `install-tailscale.sh` | Installs [Tailscale](https://tailscale.com) via the official apt repository, then enables and starts `tailscaled` | No (uses sudo) |
| `install-terraform.sh` | Installs [HashiCorp Terraform](https://www.terraform.io/) via the official apt repository | No (uses sudo) |
...
| `install-virt.sh` | Installs QEMU/KVM and libvirt for virtual machine management | No (uses sudo) |
...
| `setup-hashicorp-repo.sh` | Adds the HashiCorp apt repository and GPG key (idempotent, used by Packer/Terraform scripts) | No (uses sudo) |
